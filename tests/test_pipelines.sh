...
# Run parameters
DEFAULT_IMAGE="immcantation/suite:devel"
...
@test "versions-report" {
    echo "Running tests with image: $IMAGE"
    run docker run -u $(id -u):$(id -g) -v $DATA_DIR:/data:z -v $RUN_DIR:/scratch:z \
        -e PYTHONWARNINGS=$PYTHONWARNINGS --workdir /scratch $IMAGE \
        bash -c 'versions report > versions-report.txt'
}
...
@test "preprocess-phix" {
    ...
    run docker run -u $(id -u):$(id -g) -v $DATA_DIR:/data:z -v $RUN_DIR:/scratch:z \
        -e PYTHONWARNINGS=$PYTHONWARNINGS $IMAGE \
...
@test "changeo-10x" {
    ...
    run docker run -u $(id -u):$(id -g) -v $DATA_DIR:/data:z -v $RUN_DIR:/scratch:z \
        -e PYTHONWARNINGS=$PYTHONWARNINGS $IMAGE \
        bash changeo-10x -s $READS -a $ANNOTATIONS -x $DIST \
...
@test "changeo-igblast" {
    ...
    SAMPLE=combined
    READS="/data/mixed-data/test_combined_repertoire.fasta"
