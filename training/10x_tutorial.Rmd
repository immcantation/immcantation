...
```{r}
heavy <- dplyr::filter(data_mut, locus == "IGH")
light <- dplyr::filter(data_mut, locus != "IGH")

comb <- resolveLightChains(heavy, light)

clones <- formatClones(comb,
                       chain = "HL", traits = c("day", "isotype", "gex_annotation"),
                       num_fields = c("duplicate_count"), minseq = 5)
clones
```
...
trees <- getTrees(clones, build = "igphyml", exec = "/usr/local/igphyml/src/igphyml", nproc=2)
head(trees)
```
