...
# 'head' can be used to see the first few (default is 5, here its's one) rows of an object
...
# this can be used once the new version of Dowser is released. If it's desired a chunk can also be shown how to make clones from just the light chains. 
...
# more release related content. Once it's release the echo=FALSE can be deleted
...
# RAxML
trees <- getTrees(clones, build = "raxml", exec = "/user/local/raxml-ng/bin/raxml-ng", partition = "scaled")
# IgPhML
trees <- getTrees(clones, build = "igphyml", exec = "/user/local/igphyml/src/igphyml", partition = "hl")
```
