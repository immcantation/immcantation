...
```{r}
# read in the data
data <- airr::read_rearrangement(file.path("results",
...
cat(paste("There are", nrow(data), "rows in the data after filtering out cells
...
```{r}
gex_db <- readRDS(file.path("..", "data", "bcr_phylo_tutorial", "GEX.data.rds"))
```
...
plot(estimateAbundance(results), colors = "steelblue", silent = TRUE) +
  theme(axis.title = element_text(size = 18))
```
