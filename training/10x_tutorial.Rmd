...
Note that you might need to install several Bioconductor packages that are dependencies for some of the R-based Immcantation packages.
...
It is possible to subset columns using regular `R` functions. The following code shows how to subset some fields of interest for the first sequence in the table:
...
### Run hierarchicalClones

After we decide the threshold for calling clones, the `hierarchicalClones` function in `SCOPer` package can be used to call clones using single cell mode:

```{r, echo=FALSE}
bcr_data <- select(bcr_data, -c("clone_id"))
```
...
### Visualize clone size distributions
