...
# If it is desired a chunk can also be shown how to make clones from just the light chains.
...
# There is an additional step that occurs before the `formatClones` step in this situation.
# In order for the trees to best use the addition of light chain information, we will need to assign `clone_subgroups` using dowser's function `resolveLightChains`.
# This group cells within a clone based on the light chain V and J gene and assign a subgroup to each sequence. Then, in the `formatClones` step, speciy `chain="HL"`.
...
# RAxML
trees <- getTrees(clones, build = "raxml",
                  exec = "/user/local/bin/raxml-ng", partition = "scaled")
# IgPhML
trees <- getTrees(clones, build = "igphyml",
                  exec = "/user/local/share/igphyml/src/igphyml", partition = "hl")
```
