...
bcr_data_selected <- bcr_data %>%
                       select(cell_id_unique, clone_id, mu_freq, isotype) %>%
...
```{r, fig.dim = c(7, 6)}
anno_cols <- c("GC B" = "dodgerblue2", "Naive B" = "darkgoldenrod2",
               "PB" = "firebrick2", "RMB" = "plum2")

ggplot() +
  geom_point(data = bcr_gex_data,
...
# base plot
p <- ggplot() +
       geom_point(data = dplyr::filter(bcr_gex_data, gex_annotation != "GC B"),
...
```{r}
# isotype information
p + geom_point(data = dplyr::filter(bcr_gex_data, gex_annotation == "GC B"),
