...
# temporary cell id fix
bcr_data$cell_id_unique <- paste(bcr_data$sample, bcr_data$cell_id, sep = "_")
...
UMAPPlot(object = gex_obj, cells.highlight = highlighted_cells,
  ...
  scale_color_manual(labels = c("non-BCR", "BCR"),
                     values = c("gray", "red"))
```
...
Using the annotated information, we can layout cells from the BCR data in a UMAP
plot color-coded by the B cell subtypes.

```{r, fig.dim = c(7, 6)}
anno_cols <- c("ABC" = "green",
               "GC" = "dodgerblue2",
...
p + geom_point(data = dplyr::filter(bcr_gex_data, gex_annotation == "GC"),
               aes(x = gex_umap_1, y = gex_umap_2, color = c_gene), size = 1)
```
